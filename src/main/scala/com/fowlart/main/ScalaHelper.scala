...
class ScalaHelper {
  ...
  def getBucketContent(cart: java.util.Set[Item]): String = {

    val items = cart.toArray.toList.asInstanceOf[List[Item]]

    items.map(i =>
      s"""
         |<p class="items">${i.name()} ${i.price()} грн <input class="qty-input" value="${i.getQty}" placeholder="кількість від 1 шт" type="number" min="1" onchange="changeQty('${i.id()}',this)"><button class="remove-item-button" onclick="deleteElement('${i.id()}')">x</button></p>
         |""".stripMargin)
      .mkString("")
  }
