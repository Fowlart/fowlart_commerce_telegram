...
class ScalaHelper {
  ...
  def getBucketContent(cart: java.util.List[String]): String = {
    ...
    items.map(i =>
      s"""
         |<p class="items">$i<input class="qty-input" value="10" placeholder="кількість від 1 шт" type="number" min="1" onchange="sendItems()"><button class="remove-item-button" onclick="deleteElement('$i')">x</button></p>
         |""".stripMargin)
      .mkString("\n")
  }
