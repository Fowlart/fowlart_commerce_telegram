...
public class FileTransferSftp {
    ...
    public FileTransferSftp(@Value("${ft.sftp.login}") String login,
                            @Value("${ft.sftp.password}") String password,
                            @Value("${ft.sftp.folder}") String path) {
    ...
    private void start() throws IOException {
        ...
        sshd.start();
        LoggerHelper.logInfoInFile("SFTP server for file transfer started");
    }
}
